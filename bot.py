...
def build_boss_embed(raid, lb, members=None):
    ...
    # Unicode HP bar: filled = damage dealt, empty = HP remaining
    bar_width = 20
    filled = max(0, min(bar_width, round((total / boss["max_hp"] * bar_width) if boss["max_hp"] else 0)))
    empty = bar_width - filled
    hp_bar = "\u2588" * filled + "\u2591" * empty
    hp_left = max(0.0, boss["max_hp"] - total)
    hp_val = f"`[{hp_bar}]` **{round(hp_remaining_pct, 1)}% HP remaining**\n{fmt(hp_left)} HP left"

    status_line = "**Boss Defeated**" if defeated else "**Boss Survived**"

    # Build participant lines outside a code block so markdown renders
    participant_lines = []
    if not entries:
        participant_lines.append("No participants")
    else:
        for i, e in enumerate(entries):
            ...
            line = f"#{i+1} {name} - {dmg_str} ({pct_val}%) - {dps_val}/s"
            dead = (e["damage_dealt"] == 0)
            if dead:
                line = f"~~{line}~~"
            elif i < 3:
                line = f"**{line}**"
    ...
    embed = {
        ...
        "fields": [
            ...
            {"name": "Initiated by", "value": init_name, "inline": True},
        ],
